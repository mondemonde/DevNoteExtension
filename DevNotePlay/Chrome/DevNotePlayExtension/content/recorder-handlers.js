...
Recorder.addEventHandler('clickAt', 'click', function(event) {
    if (event.button == 0 && !preventClick && event.isTrusted) {
        if (!preventClickTwice) {
            ...
            var coor = "#0#" +  event.target.textContent +"#0#" +  x + "###" + y +"Delay3";
            this.record("click", this.locatorBuilders.buildAll(event.target), coor);


            //this.record("click", this.locatorBuilders.buildAll(event.target), event.target.textContent);
            
            //this.record("click", this.locatorBuilders.buildAll(event.target), event.target.innerHTML);
